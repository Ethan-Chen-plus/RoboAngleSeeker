...
class CEM:
    ...
    def optimize(self, state, init_mean, init_var):
        ...
        for _ in range(5):
            ...
            # Calculate the cumulative reward for each action sequence
            returns = self.fake_env.propagate(state, action_sequences)[:, 0]
            # Select the action sequences with the highest cumulative rewards
            elites = action_sequences[np.argsort(
                returns)][-int(self.elite_ratio * self.n_sequence):]
            new_mean = np.mean(elites, axis=0)
            new_var = np.var(elites, axis=0)
            # Update the distribution of action sequences
...
class FCLayer(nn.Module):
    ''' Fully connected layer for the ensemble '''
...
class EnsembleModel(nn.Module):
    ''' Environmental model integration '''
    def __init__(self,
                 state_dim,
                 action_dim,
                 ensemble_size=5,
        ...
        # The output includes mean and variance, therefore it is twice the sum of the state and reward dimensions
        ...
        self.layer5 = FCLayer(200, self._output_dim, ensemble_size,
                              nn.Identity())
        self.apply(init_weights)  # Initialize parameters in the environment model
    ...
    def forward(self, x, return_log_var=False):
        ...
        # In the PETS algorithm, variance is controlled between the minimum and maximum values
...
class EnsembleDynamicsModel:
    ''' Integration of environmental models with refined training '''
    ...
    def train(self,
        ...
        for epoch in itertools.count():
            ...
            with torch.no_grad():
                ...
                if break_condition or epoch > max_iter:  # end training
                    break
...
class PETS:
    ...
    def mpc(self):
        ...
        self._env.action_time.append(time.time()-env.start)
    ...
    def train(self):
        return_list = []
        explore_return = self.explore()  # Exploring random strategies to collect data from a sequence
